...
type CanvasRenderingContext2D with
    ...
    member this.drawColumn(x: int, columnHeight: int) =
        this.lineWidth <- 1.0
        this.beginPath()
        this.moveTo(x, this.height/2 - columnHeight/2)
        this.lineTo(x, this.height/2 + columnHeight/2)
        this.stroke()
...
type Player =
    ...
    static member SPEED = 32<1/s>
    static member TURN = 2<rad/s>
...
let init () =
    let x = 2.5 * float Tile.SIZE
    let y = 2.5 * float Tile.SIZE
    let o = Empty
    ...
    { Map =
        Map.make
            (16, 9)
            [| W; W; W; W; W; W; o; o; o; W; o; o; o; o; W; o
               W; o; o; o; o; o; o; o; o; W; o; o; o; o; W; o
               D; o; o; o; o; o; o; o; W; D; W; W; W; W; D; W
               W; o; o; o; o; o; o; o; o; W; o; o; o; o; W; o
               W; W; W; W; W; W; o; o; o; W; o; o; o; o; W; o
               W; o; o; o; o; o; o; o; o; W; o; o; o; o; W; o
               W; o; o; o; o; o; o; o; W; D; W; W; W; W; D; W
               W; o; o; o; o; o; o; o; o; W; o; o; o; o; W; o
               W; o; o; o; o; o; o; o; o; W; o; o; o; o; W; o |]
...
let update dt game =
    ...
    // scale by timestep
    let dist = f speed / ms.perSecond * dt
    let turn = f steer / ms.perSecond * dt
    ...
    let x, y =
        Math.Clamp(x, 0.0, float ((map.Width - 1) * Tile.SIZE)),
        Math.Clamp(y, 0.0, float ((map.Height - 1) * Tile.SIZE))
...
let draw state (ctx: Canvas) =
    ...
    let px, py, dir = int player.X, int player.Y, player.Dir
    let fov = Player.FOV
    let map = state.Map
    ...
    for column in 0 .. w - 1 do
        ...
        let angle =
            ...
            dir + f pixelsFromCenter * radPerPixel
        ...
        let intersectX =
            if dx = 0 then
                ...
                let x' = px - px % Tile.SIZE + (if dx > 0 then Tile.SIZE else 0)
                let y' = py + dy * (x' - px) / dx
                Some (x', y')
        let intersectY =
            if dy = 0 then
                ...
                let y' = py - py % Tile.SIZE + (if dy > 0 then Tile.SIZE else 0)
                let x' = px + dx * (y' - py) / dy
                Some (x', y')

        // march the ray forward until something is hit (or map limit reached)
        let stepY = int <| float Tile.SIZE * Math.Tan(float angle)
        let stepY = Math.Abs(stepY) * Math.Sign(dy)
        let stepX = int <| float Tile.SIZE * Math.Tan(Math.PI / 2.0 - float angle)
        let stepX = Math.Abs(stepX) * Math.Sign(dx)
        let rec rayMarch x y intersectX intersectY =
            let i, j =
                y / Tile.SIZE + (if y % Tile.SIZE = 0 && dy < 0 then -1 else 0),
                x / Tile.SIZE + (if x % Tile.SIZE = 0 && dx < 0 then -1 else 0)
            if j < 0 || j >= map.Width || i < 0 || i >= map.Height then
                None
            else
                match map.[i, j] with
                | Wall _ as w -> Some (w, x, y)
                | Empty -> // move to the closest grid intersection point
                    let distX =
                        if Option.isSome intersectX then
                            distance x y <|| Option.get intersectX
                        else
                            infinity
                    let distY =
                        if Option.isSome intersectY then
                            distance x y <|| Option.get intersectY
                        else
                            infinity
                    if distX <= distY then
                        let x', y' = Option.get intersectX
                        rayMarch x' y' (Some (x' + Math.Sign(dx)*Tile.SIZE, y' + stepY)) intersectY
                    else
                        let x', y' = Option.get intersectY
                        rayMarch x' y' intersectX (Some (x' + stepX, y' + Math.Sign(dy)*Tile.SIZE))
        let hit = rayMarch px py intersectX intersectY

        // draw a single vertical column based on what was hit
        match hit with
        | Some (Wall color, hitX, hitY) ->
            let height = 2048.0 / distance px py hitX hitY
            ctx.color <- color
    ...
    ctx.beginPath()
    ctx.moveTo(px, py)
    let tx, ty = step px py 16.0 dir in ctx.lineTo(int tx, int ty)
    ctx.stroke()
